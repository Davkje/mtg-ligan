...
export default function MatchList({ matches, players }: Props) {
  ...
  function changeEditPlayerCount(count: PlayerCount) {
    setEditState((prev) =>
      prev ? { ...prev, playerCount: count, assignments: { 1: "", 2: "", 3: "", 4: "", 5: "" } } : prev
    );
  }
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {!adminAuthed ? (
          <button
            ...
            className="text-xs text-foreground/50 hover:text-accent border border-border rounded px-3 py-1.5 transition-colors"
          >
            Manage matches
          </button>
        ) : (
          ...
          <div className="bg-surface border border-border rounded-lg p-6 w-full max-w-sm shadow-xl">
            ...
            <form onSubmit={handleAdminLogin} className="space-y-3">
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => { setShowAdminModal(false); setAdminAuthError(""); }}
      ...
      {showMasterModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="bg-surface border border-border rounded-lg p-6 w-full max-w-sm shadow-xl">
            ...
            <p className="text-sm text-foreground/50 mb-4">Deleting a match requires the master password.</p>
            <form onSubmit={handleMasterLogin} className="space-y-3">
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => { setShowMasterModal(false); setPendingDeleteId(null); setMasterAuthError(""); }}
                  className="flex-1 rounded border border-border px-3 py-2 text-sm hover:bg-background transition-colors"
                >
                  Cancel
                </button>
      ...
      {confirmDeleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="bg-surface border border-border rounded-lg p-6 w-full max-w-sm shadow-xl">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => { setConfirmDeleteId(null); setActionError(""); }}
                className="flex-1 rounded border border-border px-3 py-2 text-sm hover:bg-background transition-colors"
              >
                Cancel
              </button>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setEditState((s) => s ? { ...s, playedAt: e.target.value } : s)}
                    ...
                    <div>
                      <label className="block text-xs text-foreground/50 mb-1">Number of players</label>
                      <div className="flex gap-2">
                      ...
                      {Array.from({ length: editState.playerCount }, (_, i) => i + 1).map((p) => (
                        <div key={p} className="flex items-center gap-3">
                          <span className="w-20 text-xs font-semibold shrink-0">
                            {PLACEMENT_LABEL[p]}
                            <span className="ml-1 text-accent">+{POINTS_TABLE[editState.playerCount][p - 1]}</span>
                          </span>
                          <select
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => { setEditingId(null); setEditState(null); setActionError(""); }}
